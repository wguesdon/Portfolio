#!/usr/bin/env python
"""SageMaker entry point: LightGBM shallow + Ordered Target Encoding for PS6E4.

Very shallow trees (max_depth=2) with many estimators (3000). Produces a very
different bias-variance tradeoff from the standard lgb_ote (depth=6). Each tree
captures only simple interactions, so the model relies on additive combination
of many weak learners. Good diversity for stacking.
Saves oof_lgb_ote_shallow.npy and pred_lgb_ote_shallow.npy.
"""
import os
import gc
import warnings
...
if __name__ == "__main__":
    print("=" * 60)
    print("LightGBM SHALLOW + Ordered Target Encoding")
    print("=" * 60)
    ...
    # Shallow LGB: depth 2, many trees, slower learning rate
    lgb_params = {
        "n_estimators": 3000,
        "num_leaves": 4,           # 2^2 = 4 leaves for depth 2
        "max_depth": 2,
        "learning_rate": 0.03,     # slower LR to compensate for weak trees
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X, y), 1):
        ...
        model.fit(
            ...
            callbacks=[
                __import__("lightgbm").early_stopping(200, verbose=True),
    ...
    np.save(os.path.join(MODEL_DIR, "oof_lgb_ote_shallow.npy"), oof_reordered.astype(np.float32))
    np.save(os.path.join(MODEL_DIR, "pred_lgb_ote_shallow.npy"), pred_reordered.astype(np.float32))

    print(f"\nSaved oof_lgb_ote_shallow.npy {oof_reordered.shape}")
    print(f"Saved pred_lgb_ote_shallow.npy {pred_reordered.shape}")
