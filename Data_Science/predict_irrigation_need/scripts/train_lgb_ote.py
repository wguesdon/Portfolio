#!/usr/bin/env python
"""SageMaker entry point: LightGBM + Ordered Target Encoding for PS6E4.

Same OTE pipeline as train_xgb_ote.py but with LightGBM classifier.
Uses StratifiedKFold(seed=42) matching our ensemble fold strategy.
Saves oof_lgb_ote.npy and pred_lgb_ote.npy with our label encoding (High=0, Low=1, Medium=2).
"""
import os
import gc
import warnings
...
# ============================================================
# ORDERED TARGET ENCODER (same as train_xgb_ote.py)
# ============================================================
...
# ============================================================
# FEATURE ENGINEERING (same as train_xgb_ote.py)
# ============================================================
...
if __name__ == "__main__":
    print("=" * 60)
    print("LightGBM + Ordered Target Encoding")
    print("=" * 60)
    ...
    np.save(os.path.join(MODEL_DIR, "oof_lgb_ote.npy"), oof_reordered.astype(np.float32))
    np.save(os.path.join(MODEL_DIR, "pred_lgb_ote.npy"), pred_reordered.astype(np.float32))

    print(f"\nSaved oof_lgb_ote.npy {oof_reordered.shape}")
    print(f"Saved pred_lgb_ote.npy {pred_reordered.shape}")
