#!/usr/bin/env python
"""SageMaker entry point: XGBoost + OTE + Magic Formula for PS6E4.

Combines the OTE pipeline (digit extraction + ordered target encoding) with the
magic formula features (threshold flags, composite scores, boundary distances).
Saves oof_xgb_ote_magic.npy and pred_xgb_ote_magic.npy with our label encoding.
"""
import os
import gc
import warnings
...
def add_magic_features(df):
    """Add magic formula features from Chris Deotte's exact formula discovery."""
    df["magic_soil_dry"] = (df["Soil_Moisture"] < 25).astype("int8")
    df["magic_rain_low"] = (df["Rainfall_mm"] < 300).astype("int8")
    df["magic_temp_hot"] = (df["Temperature_C"] > 30).astype("int8")
    df["magic_wind_high"] = (df["Wind_Speed_kmh"] > 10).astype("int8")
    df["magic_harvest"] = (df["Crop_Growth_Stage"] == "Harvest").astype("int8")
    df["magic_sowing"] = (df["Crop_Growth_Stage"] == "Sowing").astype("int8")
    df["magic_mulch_yes"] = (df["Mulching_Used"] == "Yes").astype("int8")
    ...
    # Predicted class from exact formula (as categorical feature)
    df["magic_pred"] = np.where(
        df["magic_score"] <= 0, 0,
        np.where(df["magic_score"] >= 4, 2, 1)
    ).astype("int8")
    return df


def feature_engineering(train_df, test_df, nums, cats):
    """Digit extraction + adaptive rounding + magic formula + frequency encoding."""
    M = train_df[nums].max()
    ...
    # Add magic formula features (must be before frequency encoding)
    train_df = add_magic_features(train_df)
    test_df = add_magic_features(test_df)

    magic_cols = [c for c in train_df.columns if c.startswith("magic_")]
    print(f"  Added {len(magic_cols)} magic formula features")
    ...
    # Include digit features and magic int8 flags as categoricals for OTE
    magic_cat = [c for c in test_df.columns if c.startswith("magic_") and test_df[c].dtype == np.int8]
    category_cols = cats + [c for c in test_df.columns if "digit" in c] + magic_cat
    ...
    # Include magic numeric features (margins, scores, boundary_dist)
    magic_num = [c for c in test_df.columns if c.startswith("magic_") and c not in magic_cat]
    features = category_cols + nums + magic_num
...
if __name__ == "__main__":
    print("=" * 60)
    print("XGBoost + Ordered Target Encoding + Magic Formula")
    print("=" * 60)
    ...
    np.save(os.path.join(MODEL_DIR, "oof_xgb_ote_magic.npy"), oof_reordered.astype(np.float32))
    np.save(os.path.join(MODEL_DIR, "pred_xgb_ote_magic.npy"), pred_reordered.astype(np.float32))

    print(f"\nSaved oof_xgb_ote_magic.npy {oof_reordered.shape}")
    print(f"Saved pred_xgb_ote_magic.npy {pred_reordered.shape}")
