#!/usr/bin/env python
"""SageMaker entry point: LightGBM deep + Ordered Target Encoding for PS6E4.

Deep trees (max_depth=12) with more leaves and stronger regularization.
Captures higher-order feature interactions that shallow models miss.
Very different bias-variance tradeoff from standard lgb_ote (depth=6).
Saves oof_lgb_ote_deep.npy and pred_lgb_ote_deep.npy.
"""
import os
import gc
import warnings
...
if __name__ == "__main__":
    print("=" * 60)
    print("LightGBM DEEP + Ordered Target Encoding")
    print("=" * 60)
    ...
    # Deep LGB: depth 12, more leaves, stronger regularization to control overfitting
    lgb_params = {
        "n_estimators": 1500,
        "num_leaves": 256,         # more leaves for deep trees
        "max_depth": 12,
        "learning_rate": 0.05,
        "colsample_bytree": 0.6,   # stronger column sampling to reduce overfitting
        "subsample": 0.7,          # stronger row sampling
        "subsample_freq": 1,
        "reg_alpha": 5.0,          # stronger L1 regularization
        "reg_lambda": 10.0,        # stronger L2 regularization
        "min_child_samples": 50,   # more samples per leaf to prevent overfitting
        "max_bin": 10000,
        "random_state": 2026,
        "n_jobs": -1,
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X, y), 1):
        ...
        train_w = te_train["weight"].values
        ...
        model.fit(
            ...
            callbacks=[
                __import__("lightgbm").early_stopping(100, verbose=True),
    ...
    np.save(os.path.join(MODEL_DIR, "oof_lgb_ote_deep.npy"), oof_reordered.astype(np.float32))
    np.save(os.path.join(MODEL_DIR, "pred_lgb_ote_deep.npy"), pred_reordered.astype(np.float32))

    print(f"\nSaved oof_lgb_ote_deep.npy {oof_reordered.shape}")
    print(f"Saved pred_lgb_ote_deep.npy {pred_reordered.shape}")
