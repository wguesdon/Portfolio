#!/usr/bin/env python
"""SageMaker entry point: ExtraTrees + Ordered Target Encoding for PS6E4.

Based on train_xgb_ote.py pipeline. ExtraTrees uses bagging with random split
points, producing very different errors from boosting models. Fast on CPU.
Saves oof_et_ote.npy and pred_et_ote.npy with label encoding (High=0, Low=1, Medium=2).
"""
import os
import gc
import warnings
...
if __name__ == "__main__":
    print("=" * 60)
    print("ExtraTrees + Ordered Target Encoding")
    print("=" * 60)
    ...
    # ExtraTrees params: many trees, all cores, class_weight for imbalance
    et_params = {
        "n_estimators": 2000,
        "max_depth": None,
        "min_samples_leaf": 5,
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X, y), 1):
        ...
        # Fill NaN from OTE transform for sklearn compatibility
    ...
    np.save(os.path.join(MODEL_DIR, "oof_et_ote.npy"), oof_reordered.astype(np.float32))
    np.save(os.path.join(MODEL_DIR, "pred_et_ote.npy"), pred_reordered.astype(np.float32))

    print(f"\nSaved oof_et_ote.npy {oof_reordered.shape}")
    print(f"Saved pred_et_ote.npy {pred_reordered.shape}")
