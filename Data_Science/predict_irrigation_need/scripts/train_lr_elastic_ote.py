#!/usr/bin/env python
"""LogisticRegression with ElasticNet penalty (L1 + L2 mix) + OTE for PS6E4.

Distinct from the existing LR variants in the ensemble:
  - lr_ote: sklearn L2, C=1.0
  - lr_cuml: cuML L2, C=0.1
  - lr_l1_cuml: cuML L1, C=0.1
  - lr_elastic (this): sklearn ElasticNet, C=0.5, l1_ratio=0.5

Runs on CPU, ~30-60 min on a 12-core i7.

Inputs:
  INPUT_DIR (env SM_CHANNEL_TRAINING) - directory with train.csv and test.csv
  MODEL_DIR (env SM_MODEL_DIR)       - directory to write oof/pred .npy

Outputs:
  oof_lr_elastic.npy, pred_lr_elastic.npy (label order: High=0, Low=1, Medium=2)
"""
import os
import gc
import warnings
...
if __name__ == "__main__":
    print("LR ElasticNet (C=0.5, l1_ratio=0.5) + OTE")
    ...
    # Use numeric dtype check (works across pandas versions where string columns
    # may report either 'object' or 'str' dtype)
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X, y), 1):
        ...
        X_train_np = scaler.fit_transform(X_train_te)
        X_val_np = scaler.transform(X_val_te)
        X_test_np = scaler.transform(X_test_te)

        print(f"  Training LR ElasticNet on {X_train_np.shape[0]} x {X_train_np.shape[1]}")

        # ElasticNet via saga solver (supports L1 + L2 combo)
    ...
    # Reorder to High=0, Low=1, Medium=2
    reorder = [target2idx[label] for label in le.classes_]
    oof_reordered = oof_preds[:, reorder]
    pred_reordered = test_preds[:, reorder]

    os.makedirs(MODEL_DIR, exist_ok=True)
    np.save(os.path.join(MODEL_DIR, "oof_lr_elastic.npy"), oof_reordered.astype(np.float32))
    np.save(os.path.join(MODEL_DIR, "pred_lr_elastic.npy"), pred_reordered.astype(np.float32))
