#!/usr/bin/env python
"""SageMaker entry point: KNN + Ordered Target Encoding for PS6E4.

Based on train_xgb_ote.py pipeline. KNN uses distance-based decision boundaries,
maximally different from tree-based models. Slow on 630K rows but doable on c5.9xlarge.
Saves oof_knn_ote.npy and pred_knn_ote.npy with label encoding (High=0, Low=1, Medium=2).
"""
import os
import gc
import warnings
...
if __name__ == "__main__":
    print("=" * 60)
    print("KNN + Ordered Target Encoding")
    print("=" * 60)
    ...
    # Prepare data (no sample weights for KNN)
    ...
    # KNN does NOT use 4x shuffle OTE augmentation (no training phase).
    # Instead, fit OTE once per fold and apply to train/val/test.
    # We still use the OTE encoder for feature creation.
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X, y), 1):
        ...
        # Ordered Target Encoding: single fit (no 4x shuffle for KNN since
        # there is no training iteration that benefits from augmentation)
        te = OrderedTE()
        ...
        X_train_te = te.train.drop([TARGET_COL], axis=1)
        y_train_te = te.train[TARGET_COL]
        ...
        # Fill NaN and scale (critical for KNN)
        ...
        # Train KNN with multiple k values and pick best
        best_k_score = 0
        best_k = 15
        for k in [7, 11, 15, 21, 31]:
            ...
            model.fit(X_train_scaled, y_train_te)
            k_pred = model.predict_proba(X_val_scaled)
            k_score = balanced_accuracy_score(y_val, np.argmax(k_pred, axis=1))
            print(f"    k={k:3d}  fold {fold} BA: {k_score:.5f}")
            if k_score > best_k_score:
                best_k_score = k_score
                best_k = k

        print(f"  Best k={best_k} for fold {fold}")
        ...
        test_preds += model.predict_proba(X_test_scaled) / N_FOLDS
    ...
    np.save(os.path.join(MODEL_DIR, "oof_knn_ote.npy"), oof_reordered.astype(np.float32))
    np.save(os.path.join(MODEL_DIR, "pred_knn_ote.npy"), pred_reordered.astype(np.float32))

    print(f"\nSaved oof_knn_ote.npy {oof_reordered.shape}")
    print(f"Saved pred_knn_ote.npy {pred_reordered.shape}")
