#!/usr/bin/env python
"""SageMaker entry point: Logistic Regression + Ordered Target Encoding for PS6E4.

Based on train_xgb_ote.py pipeline. Linear model produces fundamentally different
decision boundaries from all tree-based models. Fast on CPU.
Saves oof_lr_ote.npy and pred_lr_ote.npy with label encoding (High=0, Low=1, Medium=2).
"""
import os
import gc
import warnings
...
if __name__ == "__main__":
    print("=" * 60)
    print("Logistic Regression + Ordered Target Encoding")
    print("=" * 60)
    ...
    # Like KNN, LR does not benefit from 4x shuffle augmentation.
    # Use single OTE fit per fold.
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X, y), 1):
        ...
        # Ordered Target Encoding: single fit
        te = OrderedTE()
        ...
        X_train_te = te.train.drop([TARGET_COL, "weight"], axis=1)
        y_train_te = te.train[TARGET_COL]
        train_w_te = te.train["weight"]
        ...
        # Fill NaN and scale (critical for LR)
        ...
        model.fit(X_train_scaled, y_train_te, sample_weight=train_w_te)
    ...
    np.save(os.path.join(MODEL_DIR, "oof_lr_ote.npy"), oof_reordered.astype(np.float32))
    np.save(os.path.join(MODEL_DIR, "pred_lr_ote.npy"), pred_reordered.astype(np.float32))

    print(f"\nSaved oof_lr_ote.npy {oof_reordered.shape}")
    print(f"Saved pred_lr_ote.npy {pred_reordered.shape}")
