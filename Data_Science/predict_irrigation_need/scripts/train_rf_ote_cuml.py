#!/usr/bin/env python
"""SageMaker entry point: cuML RandomForest + Ordered Target Encoding for PS6E4.

Uses RAPIDS cuML GPU-accelerated RandomForest for 10-100x speedup over sklearn.
Bagging with optimal splits (vs ExtraTrees random splits) adds diversity.
Saves oof_rf_ote.npy and pred_rf_ote.npy with label encoding (High=0, Low=1, Medium=2).
"""
import os
import gc
import subprocess
...
if __name__ == "__main__":
    print("=" * 60)
    print("cuML RandomForest + Ordered Target Encoding")
    print("=" * 60)
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X, y), 1):
        ...
        # Fill NaN for cuML compatibility
        X_train = X_train.fillna(0)
        X_val_te = X_val_te.fillna(0)
        X_test_te = X_test_te.fillna(0)

        # Convert to float32 numpy for cuML
        X_train_np = X_train.values.astype(np.float32)
        y_train_np = y_train.values.astype(np.int32)
        X_val_np = X_val_te.values.astype(np.float32)
        X_test_np = X_test_te.values.astype(np.float32)
    ...
    np.save(os.path.join(MODEL_DIR, "oof_rf_ote.npy"), oof_reordered.astype(np.float32))
    np.save(os.path.join(MODEL_DIR, "pred_rf_ote.npy"), pred_reordered.astype(np.float32))

    print(f"\nSaved oof_rf_ote.npy {oof_reordered.shape}")
    print(f"Saved pred_rf_ote.npy {pred_reordered.shape}")
