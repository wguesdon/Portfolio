#!/usr/bin/env python
"""CatBoost deep (depth=8) + OTE for PS6E4.

Our existing cat_ote uses default depth=6 (CV 0.97919). This variant uses
depth=8 for a different error pattern. Runs on CPU.

Saves oof_cat_ote_deep.npy and pred_cat_ote_deep.npy (label order: High=0, Low=1, Medium=2).
"""
import os
import gc
import warnings
...
if __name__ == "__main__":
    print("CatBoost deep (depth=8) + OTE (CPU)")
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X, y), 1):
        ...
        train_w_aug = te_train["weight"].values
        ...
        train_pool = Pool(X_train_aug, label=y_train_aug.values, weight=train_w_aug)
        val_pool = Pool(X_val_te, label=y_val.values)
        ...
        del X_train_aug, X_val_te, X_test_te, model, train_pool, val_pool
        gc.collect()
    ...
    np.save(os.path.join(MODEL_DIR, "oof_cat_ote_deep.npy"), oof_reordered.astype(np.float32))
    np.save(os.path.join(MODEL_DIR, "pred_cat_ote_deep.npy"), pred_reordered.astype(np.float32))
