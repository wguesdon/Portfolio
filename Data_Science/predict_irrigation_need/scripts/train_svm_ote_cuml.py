#!/usr/bin/env python
"""SageMaker entry point: cuML SVM + Ordered Target Encoding for PS6E4.

Uses RAPIDS cuML GPU-accelerated SVM, which makes SVM feasible on 630K rows.
SVM produces fundamentally different decision boundaries from tree-based models.
Saves oof_svm_ote.npy and pred_svm_ote.npy with label encoding (High=0, Low=1, Medium=2).
"""
import os
import gc
import subprocess
...
# Install cuML on SageMaker GPU instance.
# 1. Uninstall the DLC's numba (lacks numba.cuda.core subpackage).
# 2. Install numba + numba-cuda together so the CUDA subpackage integrates.
# 3. Install cuml-cu12 (pulls cudf, rmm, raft, etc.).
subprocess.check_call(
    [sys.executable, "-m", "pip", "uninstall", "-y", "numba"],
    stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
)
subprocess.check_call(
    [sys.executable, "-m", "pip", "install", "--quiet",
     "numba", "numba-cuda", "cuml-cu12"],
...
if __name__ == "__main__":
    print("=" * 60)
    print("cuML SVM + Ordered Target Encoding")
    print("=" * 60)
    ...
    # cuML SVM uses OVR for multiclass. probability=True enables Platt scaling.
    # Note: cuML SVC with 4x OTE augmented training (~2M rows) may be slow.
    # We train on the original fold (no 4x augmentation) to keep it feasible,
    # but still use OTE features from the 4x-fitted encoder.
    svm_params = {
        "C": 10.0,
        "kernel": "rbf",
        "probability": True,
        "class_weight": {
            int(cls): float(w) for cls, w in weights_dict.items()
        },
    }
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X, y), 1):
        ...
        # Ordered Target Encoding: 4x shuffle for fitting, but use only
        # the original-order encoded data for SVM training (not 4x augmented)
        # to keep training time manageable.
        te = OrderedTE()
        full_df = pd.concat((X_train, y_train), axis=1)
        full_df["weight"] = train_w

        # Fit OTE with 4x shuffle to get robust statistics
        for i in range(4):
            te.fit(
        ...
        # Transform train/val/test using the fitted encoder
        ...
        print(f"  Training SVM on {X_train_np.shape[0]} samples, {X_train_np.shape[1]} features")

        model = cuSVC(**svm_params)
        model.fit(X_train_np, y_train_np)

        y_pred = model.predict_proba(X_val_np)
        if hasattr(y_pred, "get"):
            y_pred = y_pred.get()
        y_pred = np.asarray(y_pred, dtype=np.float64)
        oof_preds[val_idx] = y_pred

        test_pred = model.predict_proba(X_test_np)
        if hasattr(test_pred, "get"):
            test_pred = test_pred.get()
        test_preds += np.asarray(test_pred, dtype=np.float64) / N_FOLDS

        fold_score = balanced_accuracy_score(y_val, np.argmax(y_pred, axis=1))
        fold_scores.append(fold_score)
        print(f"  Fold {fold} balanced accuracy: {fold_score:.5f}")

        del X_train_np, X_val_np, X_test_np, model
        gc.collect()
    ...
    np.save(os.path.join(MODEL_DIR, "oof_svm_ote.npy"), oof_reordered.astype(np.float32))
    np.save(os.path.join(MODEL_DIR, "pred_svm_ote.npy"), pred_reordered.astype(np.float32))

    print(f"\nSaved oof_svm_ote.npy {oof_reordered.shape}")
    print(f"Saved pred_svm_ote.npy {pred_reordered.shape}")
